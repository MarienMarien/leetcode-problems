...
public class Solution {
    private record NodeStats(bool isBst, int nodesCount, int? maxVal, int? minVal);
    ...
    private NodeStats GetLargest(TreeNode node)
    {
        if(node == null)
        {
            return new NodeStats(true, 0, null, null);
        }
        ...
        return new NodeStats(isBst, nodesCount, right.maxVal??node.val, left.minVal??node.val);
    }
}